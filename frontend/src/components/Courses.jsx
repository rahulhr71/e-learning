           ...
           console.log(isEnrolled)
            return (
              <Link key={item._id} to={`/courses/${item._id}`}>
                <div className="flex flex-col relative h-full rounded-xl border border-gray-200 shadow-lg overflow-hidden transition-transform duration-300 hover:scale-105 cursor-pointer">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center mt-6 bg-[#F5F5F5] shadow-md w-full py-3 gap-2 rounded-3xl flex-wrap">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              key={page}
              className={`px-4 py-2 rounded-full text-sm border transition-colors duration-200 ${currentPage === page
