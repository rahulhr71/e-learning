...
const CourseOverview = () => {
  ...
  const items = course.map((video, index) => ({
    ...
    children: (
      <div
        key={video._id}
        className={`flex flex-col sm:flex-row items-center justify-between p-3 rounded-lg border ${activeLesson === video._id
      ...
      >
        ...
        <div className="flex flex-col gap-1 w-full">
          <span
            className={`${activeLesson === video._id
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className='w-full bg-black flex flex-col lg:flex-row justify-center items-center gap-6 p-4 lg:p-10'>
        ...
        <div className='flex flex-col gap-4 lg:w-2/3 w-full'>
          ...
          <h1 className='text-white font-bold text-xl sm:text-3xl'>{item.name || "Course Title"}</h1>
      ...
      <div className='w-full sm:w-4/5 m-auto mt-10'>
        ...
        <div className='p-4 bg-[#EAEAEA] rounded-b-lg text-[#555555]'>
          {activeTab === "overview" && (<div className='text-gray-700 text-sm sm:text-base leading-6'>
            <p>
              {item.overview}
            </p>
          </div>)}
